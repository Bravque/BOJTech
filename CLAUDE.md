...
- **Contact form is wired.** `components/sections/ContactForm.tsx` POSTs to `app/api/contact/route.ts`, which validates input, blocks spam via a honeypot field, and delivers email over SMTP via Nodemailer (e.g. a Hostinger mailbox). Delivery is gated on env vars (`SMTP_HOST`, `SMTP_USER`, `SMTP_PASSWORD`, optional `SMTP_PORT`/`CONTACT_FROM_EMAIL`/`CONTACT_TO_EMAIL` — see `.env.example`); when unset, submissions are validated and logged server-side (never silently lost) but no email is sent. To use a different provider (an HTTP API/CRM), replace the `deliver()` function in the route — the request/response contract is unchanged.
